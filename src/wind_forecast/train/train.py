"""
Train the seq2seq LSTM model for wind power forecasting.

Loads pre-built sequence tensors, applies z-score normalization (computed
on training set only), trains with MAE loss and early stopping, and saves
the best model checkpoint along with normalization statistics.

Usage:
    python train.py
    python train.py --data-dir data/processed/sequences --output-dir models/
    python train.py --hidden-size 128 --batch-size 256 --lr 0.0005
"""
...
def main():
    parser = argparse.ArgumentParser(description="Train wind power LSTM model.")
    parser.add_argument(
        "--data-dir",
    ...
    parser.add_argument(
        "--output-dir",
        default="../../models",
        help="Directory to save model and stats (default: models)",
    )
    ...
    # Static features: normalize capacity_mw and hub_height, leave site_id as-is
    ...
    for split_data in [train_data, val_data]:
        ...
        # Normalize only numeric static features, keep site_id
    ...
    # --- Summary ---
    # Denormalize best val loss to report in original MWh units
    best_val_mae_mwh = best_val_loss * target_std.item() + 0  # MAE doesn't need mean adjustment

    print(f"\nTraining complete.")
    print(f"  Best epoch: {best_epoch}")
    print(f"  Best val MAE (normalized): {best_val_loss:.4f}")
    print(f"  Best val MAE (MWh): {best_val_mae_mwh:.2f}")
    print(f"\nSaved to {output_dir}/:")
